...
object SyntheticCarrierGenerator {
    ...
    fun darkNoise(width: Int, height: Int): Bitmap {
        ...
        for (y in 0 until height) {
            for (x in 0 until width) {
                ...
                val v = (n * 40 + 15 + rng.nextInt(12)).coerceIn(0, 255)
                bmp.setPixel(x, y, Color.rgb(v, v + rng.nextInt(5), v + rng.nextInt(8)))
            }
        }
        return bmp
    }
    ...
    fun metallicGrain(width: Int, height: Int): Bitmap {
        ...
        // Horizontal metallic streaks
        paint.strokeWidth = 1f
        repeat(height / 3) {
            val y     = rng.nextInt(height).toFloat()
            val alpha = rng.nextInt(60) + 10
            val gray  = rng.nextInt(80) + 40
            paint.color = Color.argb(alpha, gray, gray, gray + rng.nextInt(15))
            canvas.drawLine(0f, y, width.toFloat(), y + rng.nextFloat() * 4 - 2, paint)
        }
    ...
    fun cyberpunkGrid(width: Int, height: Int): Bitmap {
        ...
        while (x < width) {
            ...
            paint.color = Color.argb(alpha, 0, 180 + rng.nextInt(75), 120 + rng.nextInt(80))
        ...
        while (y < height) {
            val alpha = rng.nextInt(40) + 15
            paint.color = Color.argb(alpha, 0, 160 + rng.nextInt(95), 100 + rng.nextInt(100))
        ...
        repeat(width * height / 400) {
            ...
            canvas.drawCircle(nx.toFloat(), ny.toFloat(), rng.nextFloat() * 2 + 0.5f, paint)
        }
